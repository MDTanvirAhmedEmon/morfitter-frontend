...
const AddSessionContentModal = ({ handleOk, handleCancel, isModalOpen, id }) => {
    ...
    const onFinish = (values) => {
        ...
        addSession({ formData: formData, id: id }).unwrap()
            .then(() => {
    ...
    return (
        <Modal centered width={600} footer={false} open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
            <Form layout="vertical" className=' pt-10' onFinish={onFinish}>
                ...
                <Form.Item label={null}>
                    <button disabled={isLoading} className=' bg-secondary text-white px-8 py-1 rounded-md mt-10 text-lg' type="primary" htmlType="submit">
                        Submit
                    </button>
                </Form.Item>
            </Form>
